...
- **No punchy one-line jabs.** Kegan is witty but not punchy. Short staccato sentences at the end of a paragraph for rhetorical emphasis read as performative and he hates them. Concretely, avoid:
  ...
  - **Parallel-clause verdict fragments** that pass judgment on the technique just described, e.g. "Adequate for a 2022 proof-of-concept, indefensible for anything you would actually deploy." or "Fine for a hack, fatal in production." The shape is `<positive judgment for thing A>, <negative judgment for thing B>.` and it reads as a hot take the writer wants to land. If the factual sentence before it already implies the judgment (which it usually does), just delete the verdict. If the judgment genuinely adds information, fold it into a longer flowing sentence that doesn't end on the parallel beat.
  - **Trailing wry tags.** A factual setup followed by a curt two-or-three-word kicker that "lands" the punchline: "...exactly the amount of information about the original method I want it to convey, **which is none.**" / "How many times has the streaming endpoint been useful? **Zero.**" / "...and the only authentication is that you know the method name. **Which is the joke.**" The setup-then-kicker shape is the same performative beat the parallel-clause verdict has, dressed differently. State the same fact plainly inside a flowing sentence ("with no trace of what the method originally did") instead of deflating into a one-or-two-word tag.
  ...
  - Wrap the same observation into a longer flowing sentence instead, or delete it. The user's actual prose is matter-of-fact and explanatory — long sentences with em-dashes and subordinate clauses, not bullet-point one-liners.
...
- **No "clause — interjection — continuation" em-dash interruptions.** Em-dashes are a real part of Kegan's voice, but only the trailing kind: a complete clause, an em-dash, and an appositive or list that runs to the end of the sentence ("The body is a method record — a header followed by IL bytes."). The reader-hostile pattern is using *paired* em-dashes mid-sentence to splice in a parenthetical: "Cecil's view is normalised — short-form branches re-encoded, exception tables reordered — so the bytes it hands back are equivalent IL." The reader has to hold the front half of the clause open while parsing the interjection and then re-attach to the tail. Rewrite as two sentences: state the main clause cleanly, then elaborate as a follow-up. Acceptable: "Cecil's view is normalised, so the bytes it hands back are equivalent IL. Along the way it may re-encode short-form branches, recompute targets, reorder exception tables." Also keep overall em-dash density modest — Kegan uses them but not nearly as often as a first draft tends to.
