...
date: 2020-10-28
image: /blog/sha256-implementation-in-dayz-standalone/cover.png
tags: [dayz, enscript, crypto]
---
...
the most significant bit is a flag for the sign, a 1 bit represents a negative number.
When right shifting signed integers, the sign is kept, meaning the sign bit is
duplicated into the high-order bits being shifted in, so every shift to the right
on a negative number introduces more 1s on the left. Here is a great diagram that
helped me understand it.

![img](/blog/sha256-implementation-in-dayz-standalone/img-1.png) 

In this image, the b7 bit (the sign bit) is being duplicated and shifted into
the high-order bits. For unsigned integers, this right shift does not do this
duplication step. Instead, a 0 is placed into the high bit we shifted away from.
In order to recreate an unsigned right shift in DayZ, we need to undo this
duplication step. To do this, we shift right once, and clear the left most bit,
our sign bit, to 0. From this point on, any further shifts to the right will
place a 0 into the left bit, matching the functionality of unsigned integers. 
...
After we shift right once, we use `& 0x7FFFFFFF` to clear only the most
significant bit to 0, every other bit will retain its value. From this point on,
we shift right the remaining amount and return our value. 
