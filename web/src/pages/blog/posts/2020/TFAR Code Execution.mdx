...
date: 2020-07-11
image: /blog/tfar-code-execution/cover.png
...
Our first step is to get the registry, or create a new one, for `cba_keybinding_registry_v3`. 
Next, we must find our action in the registry. If our item does exist, we must set
the value in the registry for that index to an exploitable payload. Remember,
the payload must be an array within an array as `CBA_fnc_getKeybind` accesses
index 0 before returning. If our item does not exist, we just have to push it
back into the registry.

And that is it. Now if we load up a game with TFAR and press, in my example,
SHIFT, it'll execute the payload.

---

So one last thing I wanted to cover was a unique feature of this exploit. My
comments throughout the screenshots have been alluding to this. The way that
getKeybind and addKeybind interact with the keybind data array, the values in
it are actually directly linked to the profilenamespace. I covered this a while
ago with my [advanced variable hiding] post. What it allows you to do is create
a single line of code for this entire exploit.

This was shown to me by a friend, and I never actually wrote it, but here is a
rough take that may or may not work.

```sqf
((["TFAR", "LRTransmit"] call cba_fnc_getKeybind select 5) select 1) set [0,{systemChat '123';false;}]
```

In theory, setting the value retrieved here, should link directly back to the
profile, and update the profile with the new exploit. Like I said, I didn't test
this, I just know that somehow it is possible.

{/* reference links */}
[Task Force Arrowhead Radio]: http://radio.task-force.ru/en/
[Lazy Eval Exploit]: https://lystic.dev/blog/lazy-eval-execution-exploit/
[createLocation]: https://community.bistudio.com/wiki/createLocation
[advanced variable hiding]: https://lystic.dev/blog/advanced-script-variable-hiding/